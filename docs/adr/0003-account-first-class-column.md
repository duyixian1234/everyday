# ADR 0003: Account as first-class schema column
...
Everyday supports multiple accounts per module (e.g., `work` and `personal` mail accounts). Timeline aggregates events across accounts. The `source` field (`mail` / `cal` / `rss` / ...) identifies the module but not the account.

Three options were considered for representing account identity in Timeline events:
