# ADR 0007: Notion provider via local ops-log with AOP dispatch hook
...
Todo / Note / Bookmark modules support two providers: `local` (SQLite, default) and `notion` (Notion API). Timeline's pull model ([ADR 0004](0004-timeline-provider-pull-only.md)) requires providers to query data sources for events.
...
The notion TimelineProvider queries `ops_log WHERE module=? AND account=? AND occurred_at > ?` and maps rows to `TimelineEvent`.
...
3. `ref_id` and `title` are extracted from the module's JSON `Output`.
4. Write failures are non-blocking (`let _ =`, stderr warning).
...
- Violates module independence (same reasoning as [ADR 0004](0004-timeline-provider-pull-only.md) rejection of push).
...
- The AOP hook parses `Output` to extract `ref_id`/`title`. Some actions (e.g., `todo complete`) don't include `title` in their output — title defaults to empty, and timeline display derives a description from the action type.
