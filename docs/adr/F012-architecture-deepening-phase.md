...
| P1 CLI/business separation | 2 | **Done** (commits `1f37aba`, `e1d964a`, `fa72ad8` + `fe1ab51`) | ArgSpec boilerplate cut ~40–55% via `cli_action!`/`flag!` macros (all 11 modules; mail `module_arg_spec` 129→61 lines); service-layer traits with domain types + Mock direct tests: `MailBackend`/`CalBackend`/`RssBackend` (+ note/todo/bookmark `*Backend` from R016) and **`MemoryBackend`/`SearchBackend`/`TimelineBackend`/`AuthBackend`** (special modules, Phase 3 wiring, commit `fe1ab51`); dispatch() is the only Output-touching path in all modules; `config` intentionally keeps the Executor-direct shape (no domain data — pure file IO + serialization; a service trait would be Speculative Generality) |
...
| P3 Lifecycle hooks | 3 | **Done** | `Executor::initialize()/health_check()/shutdown()` default no-ops; `HealthStatus` + shared `db_health()` helper; `ModuleRegistry::{initialize_all,health_check_all,shutdown_all}`; `everyday health` root command (text/JSON, exit 0/1); 8/11 modules override `health_check` (mail/cal/note/todo/bookmark probe keyring credential, rss probes config, memory/timeline probe DB — local-only); search/auth/config use the healthy default (no local resource); main.rs wires initialize+shutdown around dispatch |
| P4 Request context | 3 | **Done** (v0.11 non-breaking + v0.12 breaking) | v0.11: `RequestContext { request_id, deadline, caller }` in `shared::request_context`; thread-local propagation (one set site in main.rs, clear after dispatch); `generate_request_id()` = `cli-<nanos>-<pid>`. v0.12: explicit-parameter form (breaking) — `&RequestContext` threaded through `Executor::execute` + middleware hooks, thread-local removed ([F013](F013-request-context-explicit-parameter.md)) |
