...
| P1 CLI/business separation | 2 | **Done** (commits `1f37aba`, `e1d964a`, `fa72ad8`) | ArgSpec boilerplate cut ~65% via `cli_action!`/`flag!` macros (all 11 modules); service-layer traits with domain types + Mock tests: `MailBackend`/`CalBackend`/`RssBackend` (+ note/todo/bookmark `*Backend` from R016); dispatch() is the only Output-touching path; special modules (timeline/search/auth/config/memory) already render-separated with direct tests |
