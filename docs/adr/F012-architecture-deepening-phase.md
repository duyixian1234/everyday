...
| P1 CLI/business separation | 2 | **Done** (commits `1f37aba`, `e1d964a`, `fa72ad8` + Phase 3 wiring commit) | ArgSpec boilerplate cut ~40–55% via `cli_action!`/`flag!` macros (all 11 modules; mail `module_arg_spec` 129→61 lines); service-layer traits with domain types + Mock direct tests: `MailBackend`/`CalBackend`/`RssBackend` (+ note/todo/bookmark `*Backend` from R016) and **`MemoryBackend`/`SearchBackend`/`TimelineBackend`/`AuthBackend`** (special modules, Phase 3 wiring); dispatch() is the only Output-touching path in all modules; `config` intentionally keeps the Executor-direct shape (no domain data — pure file IO + serialization; a service trait would be Speculative Generality) |
