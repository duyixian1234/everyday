# ADR L008: Local provider degraded event granularity (latest-state snapshot)
...
[ADR L001](L001-append-only-event-log.md) established the append-only log model where each state transition is a separate event. Local module SQLite tables store **current state snapshots**, not transition history:
...
1. **Add `updated_at` column to `todos` table** (migration via `ALTER TABLE`). `set_status` updates this column. The `notes` table already has `updated_at`.
...
4. **Deletions:** the `todo delete` action ([T002](T002-todo-delete-action.md)) does a physical `DELETE` in the local provider. There is no `deleted_at` column yet; soft-delete is deferred.

5. **Notion accounts are unaffected** — ops-log ([L007](L007-notion-ops-log.md)) records every CLI-initiated transition at execution time, so full granularity is preserved for notion accounts.
...
- Rejected: requires invasive changes to `todo_local.rs` write path, making local modules maintain their own ops-log. This contradicts [L004](L004-timeline-provider-pull-only.md)'s pull model and the AOP philosophy ([L007](L007-notion-ops-log.md)).
...
- Note "updated" frequency reflects sync frequency, not actual edit frequency. Acceptable: timeline measures "when was activity detected", not "when did every keystroke happen".
- The `todos` schema change (`updated_at` column) requires migration logic in `todo_local.rs::open()` (SQLite lacks `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`; idempotent migration via column-existence check or `CREATE TABLE` with the new column for fresh DBs).
- Future soft-delete work would add a `deleted_at` column and have the provider generate `deleted` events.

## Cross-references

- The append model this degrades gracefully from: [L001](L001-append-only-event-log.md).
- The notion alternative that preserves full granularity: [L007](L007-notion-ops-log.md).
- The delete action that needs soft-delete future work: [T002](T002-todo-delete-action.md).
- The provider abstraction that makes the degradation contained: [L004](L004-timeline-provider-pull-only.md).