...
- **M006 已实现（master，未发版）** — `mail search --cached` 本地缓存搜索（v0.18.0 目标）：显式 opt-in、stale 同步、P6 类型化输出；`email_cache::search_envelopes_scoped` 账户/文件夹 scoped。ADR [M006](./docs/adr/M006-mail-search-cached.md)。M007（`mail cache gc`）待实现。
